...
import Decoration13 from './index.vue'
...
<Decoration13 :size="${form.value.size}" :row="${form.value.row}" :column="${form.value.column}" :colors="['${form.value.color1}', '${form.value.color2}']" class="mx-auto" :style="{ width: '${form.value.width}px', height: '${form.value.height}px' }" />`)
...
<template>
  <div class="p-3">
    <DocDemoBox :copied="copied" @copy="handleCopy">
      <Decoration13 :size="form.size" :row="form.row" :column="form.column" :colors="[form.color1, form.color2]" class="mx-auto" :style="{ width: `${form.width}px`, height: `${form.height}px` }" />
    </DocDemoBox>
    <div class="mt-3">
      <DocForm>
